...
class JogoBase:
        ...
        for x, y, tile in self.camada_porta.tiles():
            if tile and tile_x == x and tile_y == y:
                ...
                if propriedades and propriedades.get("porta", False):
                    destino = propriedades.get("destino", "arroz.tmx")  # Mapa destino
    ...
    def trocar_mapa(self, novo_mapa, spawn_x=None, spawn_y=None):
        ...
        try:
            ...
            # Reposiciona inimigos (opcional, ajuste conforme necessário)
            if novo_mapa == "basic.tmx":
                self.pedro.x, self.pedro.y = 200, 350
                self.gusto.x, self.gusto.y = 700, 500
                self.pooh.x, self.pooh.y = 900, 500
            elif novo_mapa == "arroz.tmx":
                # Posições diferentes para o mapa "arroz.tmx", se necessário
                self.pedro.x, self.pedro.y = 200, 200
                self.gusto.x, self.gusto.y = 600, 400
                self.pooh.x, self.pooh.y = 800, 400
