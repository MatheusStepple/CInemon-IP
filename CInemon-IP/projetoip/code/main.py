...
class Inimigo:
    def __init__(self, x, y, nome):
        ...
        if nome == 'Pedro':
            self.cor = VERMELHO
            caminho_imagem = os.path.join("Desktop", "CInemon-IP", "projetoip" ,"graphics", "Fotos", "pedro.manhas.jpeg")
            self.imagem = pygame.image.load(caminho_imagem)
            self.imagem = pygame.transform.scale(self.imagem, (40, 60))
        elif nome == 'pooh':
            self.cor = VERDE
            caminho_imagem = os.path.join("Desktop", "CInemon-IP", "projetoip" ,"graphics", "Fotos", "pooh.png")
            self.imagem = pygame.image.load(caminho_imagem)
            self.imagem = pygame.transform.scale(self.imagem, (40, 60))
        elif nome == 'Gusto':
            self.cor = LARANJA
...
class Jogo:
    def __init__(self):
        self.estado = "menu"
        # Carrega o mapa TMX
        caminho_mapa = os.path.join("Desktop", "CInemon-IP", "projetoip" ,"data", "basic.tmx")
        ...
        # Carrega a camada de colisão
        self.camada_colisao = self.tmx_data.get_layer_by_name("colisao")
        # Cria uma matriz de colisão para verificação rápida
        self.mapa_colisao = self._criar_mapa_colisao()

    def _criar_mapa_colisao(self):
        """Cria uma matriz 2D onde True indica um tile de colisão"""
        ...
        if self.camada_colisao:
            for x, y, tile in self.camada_colisao.tiles():
                if tile:  # Se há um tile nesta posição
                    mapa_colisao[x][y] = True
        return mapa_colisao

    def verificar_colisao_tile(self, x, y):
        """Verifica se a posição (x,y) colide com um tile de colisão"""
    ...
    def verificar_colisao_personagem(self, rect):
        """Verifica colisão para todos os 4 cantos do retângulo do personagem"""
        pontos = [
            (rect.left, rect.top),      # Canto superior esquerdo
            (rect.right, rect.top),     # Canto superior direito
            (rect.left, rect.bottom),   # Canto inferior esquerdo
            (rect.right, rect.bottom)   # Canto inferior direito
        ]
    ...
    def verificar_colisao_barreiras(self):
        """Verifica colisão com tiles de colisão e reverte movimento se necessário"""
        # Cria um retângulo temporário para verificar colisão
        temp_rect = pygame.Rect(self.jogador.x, self.jogador.y, 40, 60)
        
        if self.verificar_colisao_personagem(temp_rect):
            # Se houve colisão, volta para a posição anterior
    ...
    def mapa(self):
        ...
        for layer in self.tmx_data.layers:
            ...
            # Desenha os objetos (se houver)
            if hasattr(layer, 'objects'):
                for obj in layer.objects:
                    # Desenhar o objeto de acordo com seu shape
                    if obj.shape == 'rectangle':
                        ...
                        # Renderizar o texto com o tipo 'rectangle'
                        ...
                        pygame.draw.ellipse(tela, obj.color, 
                                            pygame.Rect(
                                                ...
                                                obj.y - self.camera.y, 
                                                obj.width, obj.height))
                        # Renderizar o texto com o tipo 'ellipse'
                        ...
                        # Renderizar o texto com o tipo 'point'
                        ...
                        # Renderizar o texto com o tipo 'image'
